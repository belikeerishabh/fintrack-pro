/**
 * ledger.js [v4] — bank dropdown in mark paid
 */
...
function addClient() {
  ...
  save(); closeModal('m-client'); renderLedger(); toast('Client added');
...
function openAddWork() {
  ...
  ['w-date','w-invno','w-detail','w-price','w-notes','w-pdate'].forEach(id => {
...
function editWork(id) {
  ...
  if (e.paid) {
    ...
    populateBankSelect('w-pbank', e.paidBankId || '');
  }
  openModal('m-work');
}
...
function togglePayFields() {
  ...
  if (paid) populateBankSelect('w-pbank');
}
...
function saveWork() {
  ...
  const entry = {
    ...
    paid, status: paid ? 'Paid' : 'Unpaid',
    paidDate:    paid ? document.getElementById('w-pdate').value   : '',
    paidMethod:  paid ? document.getElementById('w-pmethod').value : '',
    paidBankId:  paid ? document.getElementById('w-pbank').value   : '',
    notes:       document.getElementById('w-notes').value,
  };
  if (editingWorkId) { const idx = DB.ledger.findIndex(e => e.id === editingWorkId); DB.ledger[idx] = entry; }
  else DB.ledger.push(entry);
  editingWorkId = null;
  save(); closeModal('m-work'); renderLedger(); toast('Work entry saved');
}

function openMarkPaid(id) {
  document.getElementById('mp-id').value   = id;
  document.getElementById('mp-date').value = today();
  populateBankSelect('mp-bank');
  openModal('m-markpaid');
}

function confirmPaid() {
  const id  = document.getElementById('mp-id').value;
  const idx = DB.ledger.findIndex(e => e.id === id); if (idx < 0) return;
  ...
  DB.ledger[idx].paidBankId = document.getElementById('mp-bank').value;
  save(); closeModal('m-markpaid'); renderLedger(); toast('Marked as paid ✓');
}
...
function renderLedger() {
  ...
  cl.innerHTML = DB.clients.length
    ? DB.clients.map(c => {
        ...
        return `<div class="client-item ${c.id === selectedClient ? 'active' : ''}" onclick="selectClient('${c.id}')">
  ...
  if (!selectedClient) { lr.innerHTML = `<div class="tbl-empty"><div class="ico">👈</div>Select a client</div>`; return; }
  const client = DB.clients.find(c => c.id === selectedClient);
  if (!client) { selectedClient = null; return; }
  const entries = DB.ledger.filter(e => e.clientId === selectedClient).sort((a,b) => b.date.localeCompare(a.date));
  ...
  lr.innerHTML = `
    ...
    <div class="tbl-wrap">
      <table><thead><tr><th>Date</th><th>Work Detail</th><th>Invoice</th><th>Price</th><th>Status</th><th>Payment Info</th><th></th></tr></thead>
      <tbody>${entries.length ? entries.map(e => `<tr>
        <td>${fmtD(e.date)}</td>
        <td style="max-width:200px"><span class="editable" onclick="editWork('${e.id}')">${esc(e.workDetail)}</span></td>
        <td>${e.invoiceNo ? `<span class="badge bp">${esc(e.invoiceNo)}</span>` : `<span class="text-muted" style="font-size:11px">—</span>`}</td>
        <td class="fw6"><span class="editable" onclick="editWork('${e.id}')">${fmt(e.price)}</span></td>
        <td>${e.paid ? `<span class="badge bg">✓ Paid</span>` : `<button class="btn btn-red btn-xs" onclick="openMarkPaid('${e.id}')">Mark Paid</button>`}</td>
        <td style="font-size:11px;color:var(--muted)">${e.paid ? `${fmtD(e.paidDate)} · ${e.paidMethod||''} · ${getBankName(e.paidBankId)}` : '—'}</td>
        <td><button class="btn btn-outline btn-xs" onclick="editWork('${e.id}')" style="margin-right:4px">✏</button><button class="btn btn-red btn-xs" onclick="del('ledger','${e.id}')">✕</button></td>
      </tr>`).join('') : `<tr><td colspan="7"><div class="tbl-empty"><div class="ico">📋</div>No entries yet</div></td></tr>`}
      </tbody></table>
    </div>`;
}
